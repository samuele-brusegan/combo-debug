"""Nodo demo 'navigate_client': client azione sano per /navigate_to_pose.
...
class NavigateClient(Node):
    """Client per l'azione NavigateToPose (simulata con Fibonacci)."""
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info("NavigateClient avviato: attendo azione /navigate_to_pose...")

    def _on_timer(self) -> None:
        """Callback del timer: invia un goal."""
        if not self._action_client.wait_for_server(timeout_sec=1.0):
            self.get_logger().warn("Azione /navigate_to_pose non disponibile, riprovo...")
            return
...
def main() -> None:
    """Punto di ingresso del nodo navigate_client."""
    rclpy.init()
    node = NavigateClient()
    try:
        rclpy.spin(node)
