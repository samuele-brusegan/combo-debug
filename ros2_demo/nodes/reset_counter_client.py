"""Nodo demo 'reset_counter_client': client sano per /reset_counter.
...
class ResetCounterClient(Node):
    """Client per il servizio ResetCounter."""

    def __init__(self) -> None:
        """Inizializza il nodo, il client e il timer."""
        super().__init__("reset_counter_client")
        self._client = self.create_client(Trigger, "reset_counter")
        self._counter = 0
        self.create_timer(3.0, self._on_timer)
        self.get_logger().info("ResetCounterClient avviato: attendo servizio /reset_counter...")

    def _on_timer(self) -> None:
        """Callback del timer: invia una richiesta."""
        if not self._client.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn("Servizio /reset_counter non disponibile, riprovo...")
            return
...
def main() -> None:
    """Punto di ingresso del nodo reset_counter_client."""
    rclpy.init()
